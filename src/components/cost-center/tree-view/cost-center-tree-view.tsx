...
import { GET_COST_CENTER_TREE_VIEW_CREQ } from "@/utils/erp-requests/clients-reqs/cost-center-reqs";
...
export default function CostCenterTreeView() {
  const tableData = useAppSelector(getTable('costCenterTable'))
  const [data, setData] = useState<TreeNode[]>([])
  const fetchData = async () => {
    const res = await CLIENT_COLLECTOR_REQ(GET_COST_CENTER_TREE_VIEW_CREQ)
  ...
  if (!data?.length) {
    return (
      <div className="w-full rounded border border-dashed border-gray-200 p-6 text-center text-sm text-gray-500 dark:border-gray-700 dark:text-gray-400">
        No costs are currently available.
      </div>
    );
  }
