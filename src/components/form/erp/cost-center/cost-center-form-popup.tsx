...
import { ADD_COST_CENTER_CREQ, EDIT_COST_CENTER_CREQ, GET_COST_CENTER_CREQ, GET_MAIN_COST_CENTER_SELECT_LIST_CREQ } from "@/utils/erp-requests/clients-reqs/cost-center-reqs"
...
type CostCenterFormState = ReturnType<typeof createInitialFormState>

export default function CostCenterFormPopup() {
  const popup = useAppSelector(selectPopup("costCenterFormPopup"))
  const updatePopup = useAppSelector(selectPopup("updateCostCenterFormPopup"))
  const dispatch = useAppDispatch()
  const [data, setData] = useState<CostCenterFormState>(createInitialFormState())
  const [loading, setLoading] = useState(false)
  const [costCenters, setCostCenters] = useState<ChartOfAccountsInterface[]>([])
  const [selectedParent, setSelectedParent] = useState<ChartOfAccountsInterface | null>(null)
  const tableData = useAppSelector(getTable('costCenterTable'))
  const handleClose = () =>{
    dispatch(
      closePopup({
        popup: "costCenterFormPopup",
      })
    )
    dispatch(
      closePopup({
        popup: "updateCostCenterFormPopup",
      })
    )
  }
  const fetchCostCenters = async () => {
    const res = await CLIENT_COLLECTOR_REQ(GET_MAIN_COST_CENTER_SELECT_LIST_CREQ)
    console.log(res);
    if(res.done) {
  ...
  //* Set Selected Parent
  useEffect(() => {
    setSelectedParent(costCenters.find((e) => e.id === data.parent_id) ?? null)
  }, [costCenters, data.parent_id])
  ...
  const refetchData = async () => {
    const refreshed = await CLIENT_COLLECTOR_REQ(GET_COST_CENTER_CREQ)
      if (refreshed?.done) {
        dispatch(
          fillTable({
            tableName: "costCenterTable",
            obj: {
              data: refreshed.data.costCenters,
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true)
    const res = await CLIENT_COLLECTOR_REQ(ADD_COST_CENTER_CREQ, { data: payload })
    setLoading(false)

    if (res.done) {
      handleOpenSnakeBar(SnakeBarTypeEnum.SUCCESS, "Cost created successfully")
      setData(createInitialFormState())
      await refetchData()
      return
    }
  ...
  const handleUpdate = async () => {
    ...
    setLoading(true)
    const res = await CLIENT_COLLECTOR_REQ(EDIT_COST_CENTER_CREQ, { data: payload, id: updatePopup.data?.id })
    setLoading(false)
    if(res.done) {
      ...
      >
        <div className="px-6 py-5">
          <div className="flex items-center justify-between">
            <h3 className="text-base font-medium text-gray-800 dark:text-white/90">Create Cost</h3>
            <button
              onClick={handleClose}
                    ...
                    <div>
                      ...
                      <div className="col-span-full">
                        ...
                        options={costCenters ? ([{
                          label: '',
                          value: ''
                        }, ...costCenters?.map((e) => ({label: e.en_name || e.ar_name, value: e.id}))]) : []} placeholder="Main Cost center" value={data.parent_id} onChange={(e) => handleData(setData, 'parent_id', e.target.value)} />
                      </div>
                    </div>
