...
import { ADD_ACTIVITY_CREQ, EDIT_ACTIVITY_CREQ, GET_ACTIVITIES_CREQ, GET_MAIN_ACTIVITIES_SELECT_LIST_CREQ } from "@/utils/erp-requests/clients-reqs/activity-reqs"
...
type ActivityFormState = ReturnType<typeof createInitialFormState>

export default function ActivityFormPopup() {
  const popup = useAppSelector(selectPopup("activityFormPopup"))
  const updatePopup = useAppSelector(selectPopup("updateActivityFormPopup"))
  const dispatch = useAppDispatch()
  const [data, setData] = useState<ActivityFormState>(createInitialFormState())
  const [loading, setLoading] = useState(false)
  const [activities, setActivities] = useState<ChartOfAccountsInterface[]>([])
  ...
  const handleClose = () =>{
    dispatch(
      closePopup({
        popup: "activityFormPopup",
      })
    )
    dispatch(
      closePopup({
        popup: "updateActivityFormPopup",
      })
    )
  }
  const fetchActivities = async () => {
    const res = await CLIENT_COLLECTOR_REQ(GET_MAIN_ACTIVITIES_SELECT_LIST_CREQ)
    console.log(res);
    if(res.done) {
  ...
  //* Set Selected Parent
  useEffect(() => {
    setSelectedParent(activities.find((e) => e.id === data.parent_id) ?? null)
  }, [activities, data.parent_id])
  ...
  const refetchData = async () => {
    const refreshed = await CLIENT_COLLECTOR_REQ(GET_ACTIVITIES_CREQ)
      if (refreshed?.done) {
        dispatch(
          fillTable({
            tableName: "activitiesTable",
            obj: {
              data: refreshed.data.activities,
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true)
    const res = await CLIENT_COLLECTOR_REQ(ADD_ACTIVITY_CREQ, { data: payload })
    setLoading(false)

    if (res.done) {
      handleOpenSnakeBar(SnakeBarTypeEnum.SUCCESS, "Activity created successfully")
      setData(createInitialFormState())
      await refetchData()
      return
    }

    handleOpenSnakeBar(SnakeBarTypeEnum.ERROR, res.message || "Failed to create activity")
  }
  const handleUpdate = async () => {
    ...
    setLoading(true)
    const res = await CLIENT_COLLECTOR_REQ(EDIT_ACTIVITY_CREQ, { data: payload, id: updatePopup.data?.id })
    setLoading(false)
    if(res.done) {
      handleOpenSnakeBar(SnakeBarTypeEnum.SUCCESS, "Activity updated successfully")
      setData(createInitialFormState())
      dispatch(closePopup({
        popup: 'updateActivityFormPopup'
      }))
      ...
      >
        <div className="px-6 py-5">
          <div className="flex items-center justify-between">
            <h3 className="text-base font-medium text-gray-800 dark:text-white/90">Create Activity</h3>
            <button
              onClick={handleClose}
                    ...
                    <div>
                      ...
                      <div className="col-span-full">
                        ...
                        options={activities ? ([{
                          label: '',
                          value: ''
                        }, ...activities?.map((e) => ({label: e.en_name || e.ar_name, value: e.id}))]) : []} placeholder="Main Activity" value={data.parent_id} onChange={(e) => handleData(setData, 'parent_id', e.target.value)} />
                      </div>
                    </div>
