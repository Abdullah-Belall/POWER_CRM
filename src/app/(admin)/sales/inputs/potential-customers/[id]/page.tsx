...
import { CUSTOMER_PROFILE_SERVER_REQ } from "@/utils/requests/server-reqs/sales-managers-reqs";

export default async function CustomerPage({params}: {params: Promise<{id: string}>}) {
  const res = await SERVER_COLLECTOR_REQ(CUSTOMER_PROFILE_SERVER_REQ, {id: (await params).id})
  if(res.done) {
  const data = res?.data
  return (
    <>
  ...
  <div>
    ...
    <div className="space-y-6">
      <div className="flex flex-col justify-between gap-6 rounded-2xl border border-gray-200 bg-white px-6 py-5 sm:flex-row sm:items-center dark:border-gray-800 dark:bg-white/3">
        ...
        <UpdateCustomerStatusBtn customer_id={data?.id} />
      </div>
        ...
        <div className="space-y-6 lg:col-span-8 2xl:col-span-9">
          <div className="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/3">
            <div className="mb-5 flex justify-between items-center">
              ...
              <AddOfferBtn customer_id={data?.id} />
            </div>
