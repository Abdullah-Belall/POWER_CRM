...
import CustomersGroupTable from "@/components/tables/erp/setting/customers-group-table";
...
export default function CustomersGroup() {
  ...
  const fetchData = async () => {
    const res = await CLIENT_COLLECTOR_REQ(GET_GROUP_SETTING_CREQ, {type: AccAnalyticEnum.CUSTOMERS});
    if(res.done) {
      dispatch(fillTable({
        tableName: 'customersGroupTable',
        obj: {
  ...
  return (
    <>
          ...
          <div className="flex flex-col gap-2 mb-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h3 className="text-lg font-semibold text-gray-800 dark:text-white/90">
                Customers Group Setting
              </h3>
            </div>
            <TableActions popup={"groupSettingFormPopup"} btn={"Create Customers Group Setting"} />
          </div>
          <div>
            <CustomersGroupTable />
          </div>
      ...
      <GroupSettingFormPopup acc_analy_type={AccAnalyticEnum.CUSTOMERS} tableName="customersGroupTable" />
