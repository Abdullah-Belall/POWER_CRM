...
import IncomeGroupTable from "@/components/tables/erp/setting/income-group-table";
...
export default function IncomeGroup() {
  ...
  const fetchData = async () => {
    const res = await CLIENT_COLLECTOR_REQ(GET_GROUP_SETTING_CREQ, {type: AccAnalyticEnum.INCOME});
    if(res.done) {
      dispatch(fillTable({
        tableName: 'incomeGroupTable',
        obj: {
  ...
  return (
    <>
          ...
          <div className="flex flex-col gap-2 mb-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h3 className="text-lg font-semibold text-gray-800 dark:text-white/90">
                Income Group Setting
              </h3>
            </div>
            <TableActions popup={"groupSettingFormPopup"} btn={"Create Income Group Setting"} />
          </div>
          <div>
            <IncomeGroupTable />
          </div>
      ...
      <GroupSettingFormPopup acc_analy_type={AccAnalyticEnum.INCOME} tableName="incomeGroupTable" />
