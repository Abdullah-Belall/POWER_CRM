...
import CreditGroupTable from "@/components/tables/erp/setting/credit-group-table";
...
export default function CreditGroup() {
  ...
  const fetchData = async () => {
    const res = await CLIENT_COLLECTOR_REQ(GET_GROUP_SETTING_CREQ, {type: AccAnalyticEnum.CREDIT_GROUP});
    if(res.done) {
      dispatch(fillTable({
        tableName: 'creditGroupTable',
        obj: {
  ...
  return (
    <>
          ...
          <div className="flex flex-col gap-2 mb-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h3 className="text-lg font-semibold text-gray-800 dark:text-white/90">
                Credit Group Setting
              </h3>
            </div>
            <TableActions popup={"groupSettingFormPopup"} btn={"Create Credit Group Setting"} />
          </div>
          <div>
            <CreditGroupTable />
          </div>
      ...
      <GroupSettingFormPopup acc_analy_type={AccAnalyticEnum.CREDIT_GROUP} tableName="creditGroupTable" />
