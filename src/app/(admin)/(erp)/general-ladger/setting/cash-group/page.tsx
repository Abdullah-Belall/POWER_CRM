...
import CashGroupTable from "@/components/tables/erp/setting/cash-group-table";
...
export default function CashGroup() {
  const dispatch = useAppDispatch()
  const fetchData = async () => {
    const res = await CLIENT_COLLECTOR_REQ(GET_GROUP_SETTING_CREQ, {type: AccAnalyticEnum.CASH_HAND});
    if(res.done) {
      dispatch(fillTable({
        tableName: 'cashGroupTable',
        obj: {
  ...
  return (
    <>
      <div>
        <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white px-4 pb-3 pt-4 dark:border-gray-800 dark:bg-white/[0.03] sm:px-6">
          <div className="flex flex-col gap-2 mb-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <TableActions popup={"groupSettingFormPopup"} btn={"Create Group Setting"} />
          </div>
          <div>
            <CashGroupTable />
          </div>
      ...
      <GroupSettingFormPopup acc_analy_type={AccAnalyticEnum.CASH_HAND} tableName="cashGroupTable" />
