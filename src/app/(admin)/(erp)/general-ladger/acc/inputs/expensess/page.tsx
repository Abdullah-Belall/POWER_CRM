...
import ExpensessFundsTable from "@/components/tables/erp/funds/expensess-funds-table"
...
export default function Expensess() {
  ...
  const fetchData = async () => {
    const res = await CLIENT_COLLECTOR_REQ(GET_FUNDS_CREQ, {type: AccAnalyticEnum.EX});
    if(res.done) {
      dispatch(fillTable({
        tableName: 'expensessFundsTable',
        obj: {
          data: res.data.funds,
  ...
  return (
    <>
      ...
      <FundFormPopup acc_analy_type={AccAnalyticEnum.EX} tableName={'expensessFundsTable'} />
