...
import CreditAccountsFundsTable from "@/components/tables/erp/funds/credit-accounts-funds-table"
...
export default function CreditAccounts() {
  ...
  const fetchData = async () => {
    const res = await CLIENT_COLLECTOR_REQ(GET_FUNDS_CREQ, {type: AccAnalyticEnum.CREDIT_GROUP});
    if(res.done) {
      dispatch(fillTable({
        tableName: 'creditAccountsFundsTable',
        obj: {
          data: res.data.funds,
  ...
  return (
    <>
      ...
      <FundFormPopup acc_analy_type={AccAnalyticEnum.CREDIT_GROUP} tableName={'creditAccountsFundsTable'} />
