...
import BankFundFormPopup from "@/components/form/erp/funds/bank-fund-form-popup";
import BanksFundTable from "@/components/tables/erp/funds/banks-funds-table";

export default function CashAtBank() {
  const dispatch = useAppDispatch()
  const fetchData = async () => {
    const res = await CLIENT_COLLECTOR_REQ(GET_FUNDS_CREQ, {type: AccAnalyticEnum.BANKS});
    if(res.done) {
      dispatch(fillTable({
      ...
      <div>
        <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white px-4 pb-3 pt-4 dark:border-gray-800 dark:bg-white/[0.03] sm:px-6">
          <div className="flex flex-col gap-2 mb-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <TableActions popup={"bankfundFormPopup"} btn={"Create Fund"} />
          </div>
          <div>
            <BanksFundTable />
          </div>
