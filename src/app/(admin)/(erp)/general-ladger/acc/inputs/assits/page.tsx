...
import AssitsFundsTable from "@/components/tables/erp/funds/assits-funds-table"
...
export default function Assits() {
  const dispatch = useAppDispatch()
  const fetchData = async () => {
    const res = await CLIENT_COLLECTOR_REQ(GET_FUNDS_CREQ, {type: AccAnalyticEnum.ASSITS});
    if(res.done) {
      dispatch(fillTable({
        tableName: 'assitsFundsTable',
        obj: {
          data: res.data.funds,
  ...
  return (
    <>
      ...
      <FundFormPopup acc_analy_type={AccAnalyticEnum.ASSITS} tableName={'assitsFundsTable'} />
